import pandas as pd
import numpy as np
import random

random.seed(1)

path = f"C://Datasets/digit-set/digit-recognizer/train.csv"
data = pd.read_csv(path)
from sklearn.datasets import load_digits

data = np.array(data)
data = (data - np.mean(data)) / np.std(data)

m, n = data.shape
np.random.shuffle(data)

valid_data = data[0:10000].T
train_data = data[10000:20000].T

X_train = train_data[1:n]
y_train = (train_data[0]).reshape(10000, 1)
X_valid = valid_data[1:n]
y_valid = valid_data[0]

print(X_train.shape)

def relu(x):
    return np.maximum(0, x)

def deriv_relu(x):
    return x > 0

def softmax(x):
    exp_x = np.exp(x)
    return exp_x / np.sum(exp_x, axis=1, keepdims=True)

def one_hot(y):
    one_hot_y = np.zeros((y.size, y.max() + 1))
    one_hot_y[np.arange(y.size), y] = 1
    one_hot_y = one_hot_y.T
    return one_hot_y

class NeuralNetwork:
    def __init__(self, layer_density=128, learning_rate=0.3, epochs=1000):
        self.epochs = epochs
        self.hidden_size = layer_density
        self.input_size = X_train.shape[0]
        self.learning_rate = learning_rate
        self.weights1 = np.random.randn(self.input_size, self.hidden_size)
        self.biases1 = np.random.randn(1, self.hidden_size)
        self.weights2 = np.random.randn(self.hidden_size, 10)
        self.biases2 = np.random.randn(1, 10)

    def forward_prop(self, X_train):
        z1 = np.dot(X_train.T, self.weights1) + self.biases1
        a1 = relu(z1)
        z2 = np.dot(a1, self.weights2) + self.biases2
        a2 = softmax(z2)
        return z1, a1, z2, a2

    def back_prop(self, X_train, y_train, z1, a1, a2):
        dz2 = abs(y_train - a2)
        dw2 = np.dot(dz2.T, a1)
        db2 = np.sum(dz2, axis=0, keepdims=True)
        da1 = np.dot(dz2, self.weights2.T)
        dz1 = da1 * deriv_relu(z1)
        dw1 = np.dot(dz1.T, X_train.T)
        db1 = np.sum(dz1, axis=0, keepdims=True)
        for i in (dz2, dw2, db2, da1, dz1, dw1, db1):
            print(f"{i}:\n", i)

        # dw1 = np.nan_to_num(dw1, nan=0.5)
        # db1 = np.nan_to_num(db2, nan=0.5)
        # dw2 = np.nan_to_num(dw2, nan=0.5)
        # db2 = np.nan_to_num(db2, nan=0.5)

        return dw1, db1, dw2, db2

    def update_params(self, dw1, db1, dw2, db2):
        print(db1.shape)
        print(db2.shape)
        self.weights1 -= (self.learning_rate * dw1).T
        self.biases1 -= (self.learning_rate * db1.T)
        self.weights2 -= (self.learning_rate * dw2).T
        self.biases2 -= self.learning_rate * db2.T

    def fit(self, X, y):
        for i in range(self.epochs):
            z1, a1, z2, a2 = self.forward_prop(X)
            dw1, db1, dw2, db2 = self.back_prop(X, y, z1, a1, a2)
            self.update_params(dw1, db1, dw2, db2)
            if i % 100 == 0:
                print("Epoch" + "=" * 25 + ">: " + f"{i}")
                break


NeuralNetwork().fit(X_train, y_train)